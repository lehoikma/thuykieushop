...
class ProductsController extends Controller
{
    ...
    {
        $product = Products::where('category_id', $id)->paginate(20);
        $category1 = Categories::where('parent_id', $id)->pluck('id')->toArray();
        if (!empty($category1)) {
            array_push($category1, (int) $id);
            $product = Products::whereIn('category_id', $category1)->paginate(20);
            return view('member.product_category', ['product' => $product, 'category1' => $category1]);
        }
        return view('member.product_category', ['product' => $product, 'id' => $id]);
    }
